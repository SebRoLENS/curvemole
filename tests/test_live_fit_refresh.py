...
from curvemole.core.fitting import CancellationToken, FitMode, FitPlan, FitSettings, _Problem
from curvemole.core.registry import default_registry


def test_default_fit_budget_is_250_evaluations() -> None:
    assert FitSettings().max_nfev == 250
    assert FitPlan(["curve"]).settings.max_nfev == 250
...
def test_problem_reports_progress_exactly_every_20_evaluations() -> None:
    ...
    plan = FitPlan([curve.id], FitMode.INDEPENDENT, FitSettings(max_nfev=250))
    ...
    assert events[0][0] == pytest.approx(20 / 250)
    assert events[1][0] == pytest.approx(40 / 250)
...
def test_gui_refreshes_plot_on_twentieth_evaluation_and_keeps_zoom(
    ...
    window._task_progress(19 / 250, "Evaluation 19")
    assert not refreshes
    window._task_progress(20 / 250, "Evaluation 20")
