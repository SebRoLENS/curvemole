# CurveMole User Manual - Preview 0.8.1

**Current manual version: 0.8.1**

CurveMole is a desktop-first, scriptable application for fitting one-dimensional
scientific curves. This manual describes the behavior of CurveMole 0.8.1 Preview.
...
> **Preview status.** CurveMole 0.8.1 is suitable for evaluation and controlled
...
version. For example, a manual marked 0.8.1 describes CurveMole 0.8.1.
...
stored in the project but is not used by the version 0.8.1 optimizer.
...
Choose how many suggestions to add. Version 0.8.1 creates Gaussian components from
...
0.8.1 does not yet provide a single comprehensive on-screen results table, so use the
...
Preview 0.8.1 uses 31 grid points and a one-parameter chi-square threshold. Failed
...
0.8.1 does not yet show an automatic recovery chooser at startup, so after an abnormal
...
> https://github.com/SebRoLENS/curvemole/releases/tag/v0.8.1
