# CurveMole User Manual - Preview 0.12.2

**Current manual version: 0.12.2**

CurveMole is a desktop-first, scriptable application for fitting one-dimensional
scientific curves. This manual describes the behavior of CurveMole 0.12.2 Preview.
...
> **Preview status.** CurveMole 0.12.2 is suitable for evaluation and controlled
...
version. For example, a manual marked 0.12.2 describes CurveMole 0.12.2.
...
```bash
chmod +x CurveMole-0.12.2-linux-x86_64.AppImage
./CurveMole-0.12.2-linux-x86_64.AppImage
```

If the system cannot mount AppImages through FUSE, use extraction mode:

```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./CurveMole-0.12.2-linux-x86_64.AppImage
```
...
```bash
shasum -a 256 CurveMole-0.12.2-macos-arm64.dmg
```

On Windows PowerShell:

```powershell
Get-FileHash .\CurveMole-0.12.2-windows-x86_64.exe -Algorithm SHA256
```
...
python -m pip install ./curvemole-0.12.2-py3-none-any.whl
curvemole gui
```
...
stored in the project but is not used by the version 0.12.2 optimizer.
...
Choose how many suggestions to add. Version 0.12.2 creates Gaussian components from
...
0.12.2 does not yet provide a single comprehensive on-screen results table, so use the
...
Preview 0.12.2 uses 31 grid points and a one-parameter chi-square threshold. Failed
...
0.12.2 does not yet show an automatic recovery chooser at startup, so after an abnormal
...
> https://github.com/SebRoLENS/curvemole/releases/tag/v0.12.2
...
## Appendix D. Preview 0.12.2 limitations
