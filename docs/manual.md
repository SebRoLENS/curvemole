# CurveMole User Manual - Preview 0.28.12

**Current manual version: 0.28.12**

CurveMole is a desktop-first, scriptable application for fitting one-dimensional
scientific curves. This manual describes the behavior of CurveMole 0.28.12 Preview.
...
> **Preview status.** CurveMole 0.28.12 is suitable for evaluation and controlled
...
version. For example, a manual marked 0.28.12 describes CurveMole 0.28.12.
...
```bash
chmod +x CurveMole-0.28.12-linux-x86_64.AppImage
./CurveMole-0.28.12-linux-x86_64.AppImage
```

If the system cannot mount AppImages through FUSE, use extraction mode:

```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./CurveMole-0.28.12-linux-x86_64.AppImage
```
...
```bash
shasum -a 256 CurveMole-0.28.12-macos-arm64.dmg
```

On Windows PowerShell:

```powershell
Get-FileHash .\CurveMole-0.28.12-windows-x86_64-setup.exe -Algorithm SHA256
```
...
python -m pip install ./curvemole-0.28.12-py3-none-any.whl
curvemole gui
```
...
stored in the project but is not used by the version 0.28.12 optimizer.
...
0.28.12 does not yet provide a single comprehensive on-screen results table, so use the
...
> Romi, S. (2026). *CurveMole: Modular Scientific Curve Fitting* (Version 0.28.12)
> [Computer software]. GitHub.
> https://github.com/SebRoLENS/curvemole/releases/tag/v0.28.12
...
## Appendix D. Preview 0.28.12 limitations
