# CurveMole User Manual - Preview 0.12.5

**Current manual version: 0.12.5**

CurveMole is a desktop-first, scriptable application for fitting one-dimensional
scientific curves. This manual describes the behavior of CurveMole 0.12.5 Preview.
...
> **Preview status.** CurveMole 0.12.5 is suitable for evaluation and controlled
...
version. For example, a manual marked 0.12.5 describes CurveMole 0.12.5.
...
```bash
chmod +x CurveMole-0.12.5-linux-x86_64.AppImage
./CurveMole-0.12.5-linux-x86_64.AppImage
```

If the system cannot mount AppImages through FUSE, use extraction mode:

```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./CurveMole-0.12.5-linux-x86_64.AppImage
```
...
```bash
shasum -a 256 CurveMole-0.12.5-macos-arm64.dmg
```

On Windows PowerShell:

```powershell
Get-FileHash .\CurveMole-0.12.5-windows-x86_64.exe -Algorithm SHA256
```
...
python -m pip install ./curvemole-0.12.5-py3-none-any.whl
curvemole gui
```
...
stored in the project but is not used by the version 0.12.5 optimizer.
...
Choose how many suggestions to add. Version 0.12.5 creates Gaussian components from
...
0.12.5 does not yet provide a single comprehensive on-screen results table, so use the
...
Preview 0.12.5 uses 31 grid points and a one-parameter chi-square threshold. Failed
...
0.12.5 does not yet show an automatic recovery chooser at startup, so after an abnormal
...
> Romi, S. (2026). *CurveMole: Modular Scientific Curve Fitting* (Version 0.12.5)
> [Computer software]. GitHub.
> https://github.com/SebRoLENS/curvemole/releases/tag/v0.12.5
...
## Appendix D. Preview 0.12.5 limitations
