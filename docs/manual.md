# CurveMole User Manual - Preview 0.2.1

**Current manual version: 0.2.1**

CurveMole is a desktop-first, scriptable application for fitting one-dimensional
scientific curves. This manual describes the behavior of CurveMole 0.2.1 Preview.
...
> **Preview status.** CurveMole 0.2.1 is suitable for evaluation and controlled
...
version. For example, a manual marked 0.2.1 describes CurveMole 0.2.1.
...
```bash
chmod +x CurveMole-0.2.1-linux-x86_64.AppImage
./CurveMole-0.2.1-linux-x86_64.AppImage
```

If the system cannot mount AppImages through FUSE, use extraction mode:

```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./CurveMole-0.2.1-linux-x86_64.AppImage
```
...
```bash
shasum -a 256 CurveMole-0.2.1-macos-arm64.dmg
```

On Windows PowerShell:

```powershell
Get-FileHash .\CurveMole-0.2.1-windows-x86_64.exe -Algorithm SHA256
```
...
python -m pip install ./curvemole-0.2.1-py3-none-any.whl
curvemole gui
```
...
stored in the project but is not used by the version 0.2.1 optimizer.
...
Choose how many suggestions to add. Version 0.2.1 creates Gaussian components from
...
objects, Python objects, and machine-readable exports. In Preview 0.2.1, the GUI does
...
0.2.1 does not yet provide a single comprehensive on-screen results table, so use the
...
Preview 0.2.1 uses 31 grid points and a one-parameter chi-square threshold. Failed
...
0.2.1 does not yet show an automatic recovery chooser at startup, so after an abnormal
...
If CurveMole contributes to published work, cite the exact version used. For 0.2.1:

> Romi, S. (2026). *CurveMole: Modular Scientific Curve Fitting* (Version 0.2.1)
> [Computer software]. Zenodo. https://doi.org/10.5281/zenodo.22114004

The repository contains `CITATION.cff`, and GitHub provides **Cite this repository**.
Future releases receive their own version DOI while remaining associated with the
same software concept.
...
## Appendix D. Preview 0.2.1 limitations
