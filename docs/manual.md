# CurveMole User Manual - Preview 0.21.0

**Current manual version: 0.21.0**

CurveMole is a desktop-first, scriptable application for fitting one-dimensional
scientific curves. This manual describes the behavior of CurveMole 0.21.0 Preview.
...
> **Preview status.** CurveMole 0.21.0 is suitable for evaluation and controlled
...
version. For example, a manual marked 0.21.0 describes CurveMole 0.21.0.
...
```bash
chmod +x CurveMole-0.21.0-linux-x86_64.AppImage
./CurveMole-0.21.0-linux-x86_64.AppImage
```

If the system cannot mount AppImages through FUSE, use extraction mode:

```bash
APPIMAGE_EXTRACT_AND_RUN=1 ./CurveMole-0.21.0-linux-x86_64.AppImage
```
...
```bash
shasum -a 256 CurveMole-0.21.0-macos-arm64.dmg
```

On Windows PowerShell:

```powershell
Get-FileHash .\CurveMole-0.21.0-windows-x86_64.exe -Algorithm SHA256
```
...
python -m pip install ./curvemole-0.21.0-py3-none-any.whl
curvemole gui
```
...
stored in the project but is not used by the version 0.21.0 optimizer.
...
0.21.0 does not yet provide a single comprehensive on-screen results table, so use the
...
Preview 0.21.0 uses 31 grid points and a one-parameter chi-square threshold. Failed
...
> Romi, S. (2026). *CurveMole: Modular Scientific Curve Fitting* (Version 0.21.0)
> [Computer software]. GitHub.
> https://github.com/SebRoLENS/curvemole/releases/tag/v0.21.0
...
## Appendix D. Preview 0.21.0 limitations
