...
\renewcommand{\maketitle}{%
  \begin{titlepage}
    ...
    \vspace{0.45cm}
    {\sffamily\Large\color{CurveMoleGrey}Preview 0.28.6\par}
    \vspace{0.55cm}
...
\makeatother
\subtitle{Preview 0.28.6}
\author{Sebastiano Romi}
\date{Updated for CurveMole 0.28.6}
...
CurveMole 0.28.6 Preview. The Markdown file is the authoritative source.
The LaTeX source and PDF edition are generated automatically from it and
must carry the same version number.

\begin{quote}
\textbf{Preview status.} CurveMole 0.28.6 is suitable for evaluation and
...
an independent document version. For example, a manual marked 0.28.6
describes CurveMole 0.28.6.
...
\bottomrule\noalign{}
\endlastfoot
Linux x86-64 & \texttt{CurveMole-VERSION-linux-x86\_64.AppImage} & Mark
executable and run \\
Windows x86-64 & \texttt{CurveMole-VERSION-windows-x86\_64-setup.exe} &
Run the per-user installer \\
Windows portable &
\texttt{CurveMole-VERSION-windows-x86\_64-portable.zip} & Extract and
run \texttt{CurveMole.exe} \\
macOS Apple Silicon & \texttt{CurveMole-VERSION-macos-arm64.dmg} & Drag
CurveMole to Applications \\
macOS Intel & \texttt{CurveMole-VERSION-macos-x86\_64.dmg} & Drag
CurveMole to Applications \\
\end{longtable}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{chmod}\NormalTok{ +x CurveMole{-}0.28.6{-}linux{-}x86\_64.AppImage}
\ExtensionTok{./CurveMole{-}0.28.6{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{APPIMAGE\_EXTRACT\_AND\_RUN}\OperatorTok{=}\NormalTok{1 }\ExtensionTok{./CurveMole{-}0.28.6{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sha256sum} \AttributeTok{{-}c}\NormalTok{ SHA256SUMS.txt }\AttributeTok{{-}{-}ignore{-}missing}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{shasum} \AttributeTok{{-}a}\NormalTok{ 256 CurveMole{-}0.28.6{-}macos{-}arm64.dmg}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Get}\OperatorTok{{-}}\NormalTok{FileHash }\OperatorTok{.}\NormalTok{\textbackslash{}CurveMole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{28}\OperatorTok{.}\FunctionTok{6}\OperatorTok{{-}}\NormalTok{windows}\OperatorTok{{-}}\NormalTok{x86\_64}\OperatorTok{{-}}\NormalTok{setup}\OperatorTok{.}\FunctionTok{exe} \OperatorTok{{-}}\NormalTok{Algorithm SHA256}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{python3} \AttributeTok{{-}m}\NormalTok{ venv \textasciitilde{}/.venv/curvemole}
\BuiltInTok{source}\NormalTok{ \textasciitilde{}/.venv/curvemole/bin/activate}
\ExtensionTok{python} \AttributeTok{{-}m}\NormalTok{ pip install ./curvemole{-}0.28.6{-}py3{-}none{-}any.whl}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{py }\OperatorTok{{-}}\NormalTok{m venv }\VariableTok{$env}\OperatorTok{:}\NormalTok{USERPROFILE\textbackslash{}}\OperatorTok{.}\FunctionTok{venv}\NormalTok{\textbackslash{}curvemole}
\OperatorTok{\&} \VariableTok{$env}\OperatorTok{:}\NormalTok{USERPROFILE\textbackslash{}}\OperatorTok{.}\FunctionTok{venv}\NormalTok{\textbackslash{}curvemole\textbackslash{}Scripts\textbackslash{}Activate}\OperatorTok{.}\FunctionTok{ps1}
\NormalTok{python }\OperatorTok{{-}}\NormalTok{m pip install }\OperatorTok{.}\NormalTok{\textbackslash{}curvemole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{28}\OperatorTok{.}\FunctionTok{6}\OperatorTok{{-}}\NormalTok{py3}\OperatorTok{{-}}\NormalTok{none}\OperatorTok{{-}}\NormalTok{any}\OperatorTok{.}\FunctionTok{whl}
...
\ExtensionTok{uv}\NormalTok{ sync }\AttributeTok{{-}{-}locked} \AttributeTok{{-}{-}group}\NormalTok{ dev}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curvemole} \AttributeTok{{-}{-}version}
\end{Highlighting}
\end{Shaded}
...
latest GitHub release at startup and once per hour while the application
remains open. The badge is:
...
finite point from the active curve. Overlay and Waterfall views use
adaptive peak-preserving downsampling and clip drawing to the visible
range for dense spectra. Single view keeps the full display resolution.
Descending x arrays may be reversed only in the temporary display copy
so that fast clipping remains available. Fitting, project storage, and
export always use the complete original arrays.
...
is active. The mouse wheel remains available during Quick Add peak
placement, spline placement, Quick Fit, and live fit refreshes. A
refresh preserves the current viewport instead of resetting a zoom
chosen by the user. When the plot has keyboard focus, Up and Down
activate the previous or next spectrum in project order, matching
navigation in the curve tree.
...
with its spectrum name. The list supports Ctrl-click and Shift-click
multi-selection, including selections across spectra. Component order
...
Copy parameter & Copy one parameter from one source function to chosen
...
reject the source value are skipped and reported. The complete copy is
undoable.

\hypertarget{quick-actions}{%
\section{5.5 Quick actions}\label{quick-actions}}

The main toolbar includes \textbf{Quick Add Function} and an adjacent
function selector. The selector contains the complete current registry:
built-ins, project formulas, reusable user formulas, and loaded plugin
functions. CurveMole remembers the last selection across sessions and
also updates it when a function is added through the ordinary Add
dialog.

For peak functions, Quick Add starts click-drag graphical placement. For
a cubic spline it starts node placement. Other functions are inserted
immediately with their default parameter values. Quick Add remains
available repeatedly until the user changes the selected function or
tool.
...
project but is not used by the version 0.28.6 optimizer.
...
the active curve. Later runs reuse the last configured fit settings.
Open Fit\ldots{} to adjust the solver or fitting mode.
...
selected ones, including the relevant function states. \textbf{Visual
only - background-subtracted} instead removes the enabled marked
background only from the plot; it never changes curve arrays, fit
inputs, or exported values.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{add} & Add the component array to the accumulated model \\
\texttt{subtract} & Subtract the component array \\
\texttt{multiply} & Multiply the accumulated model by the component
array \\
\texttt{divide} & Divide the accumulated model by the component array \\
\texttt{convolve} & Convolve using FFT and scale by median x spacing \\
\end{longtable}
...
\hypertarget{quick-add-function}{%
\section{8.4 Quick Add Function}\label{quick-add-function}}

Choose a registered function from the selector beside \textbf{Quick Add
Function} in the main toolbar. The list includes built-in peaks and
backgrounds, generic functions, project-contained formulas, reusable
formulas, and trusted plugin functions. The last choice is remembered.

\begin{itemize}
\tightlist
\item
  a peak opens continuous graphical centre/FWHM placement: add several
  peaks, then press Enter, Esc, or Finish to leave placement mode;
\item
...
Quick Add therefore uses the same registry and initialization logic as
the normal Add dialog. Selecting a function in the normal Add dialog
also makes it the next Quick Add choice.
...
\section{8.6 Adding a spline background with the
...
Choose \textbf{Model \textgreater{} Find Peaks}. The first dialog can
search positive, negative, or both signs. The next dialog selects any
registered function classified as a peak, initially preferring the
current Quick Add peak. CurveMole then:
...
the next spectrum in the source series already checked. It is
unavailable at the end of that series. Copying remains undoable. Options
include:
...
Link & Graphical \textbf{Set link\ldots{}} control; linked parameters
show their source in readable form \\
\end{longtable}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{curve\_id.component\_id.parameter\_name}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{$\{curve\_ab12.component\_cd34.center\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{$\{curve\_ab12.component\_cd34.center\} + 12.5}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{abs, sqrt, exp, log, log10,}
\NormalTok{sin, cos, tan, arcsin, arccos, arctan,}
\NormalTok{sinh, cosh, tanh,}
\NormalTok{erf, erfc,}
\NormalTok{minimum, maximum, clip, where, heaviside}
\end{Highlighting}
\end{Shaded}
...
functions to leave out. Functions already present on each target retain
their values, metadata, constraints, and order during copying; the fit
still respects their fixed/free state. A function with the same internal
identity is kept rather than duplicated. Distinct functions remain
distinct even if their names match. Target-local functions are not
carried into later spectra. The selection is also preserved when
resuming a paused sequence. This is independent of ignoring functions in
the parameter-change pause monitor.
...
state, and composition/grouping. Internal links are remapped to
corresponding target parameters. Structural metadata required by the
function, such as spline nodes and custom-formula metadata, is always
retained.
...
source. CurveMole does not silently skip a paused spectrum.
...
retained while switching scope. In the Functions panel, \textbf{Show all
functions} groups entries by series, then spectrum, then function;
headings cannot be selected as functions, and multi-selection remains
available.
...
successfully. The Python API and YAML settings additionally expose
tolerances, scaling, local method, random seed, and Differential
Evolution controls. The solver menu also offers explicit TRF, Dogbox,
and LM, plus Nelder-Mead, Powell, and L-BFGS-B. The latter three
minimize the selected loss directly; they can be useful for difficult
models but may take more evaluations. LM needs unbounded parameters and
\texttt{linear} loss. Hover over a solver to see its intended use.
...
controls relevant to the chosen solver. Numeric fields accept scientific
notation such as \texttt{1e-8}; tooltips explain each option.
...
\bottomrule\noalign{}
\endlastfoot
Automatic, TRF, Dogbox, LM & \texttt{ftol}, \texttt{xtol},
\texttt{gtol}, \texttt{x\_scale}; automatic mode also offers the local
method \\
Differential Evolution & Generations, population multiplier, mutation
minimum/maximum, recombination, relative/absolute tolerance, seed and
automatic search limits; least-squares controls apply to local
refinement \\
Nelder-Mead & Iteration limit, \texttt{xatol}, \texttt{fatol}, adaptive
coefficients and optional initial simplex \\
Powell & Iteration limit, \texttt{xtol}, \texttt{ftol} \\
L-BFGS-B & Iteration limit, \texttt{ftol}, projected-gradient tolerance,
line-search steps and correction history \\
\end{longtable}
...
Preview 0.28.6 does not yet provide a single comprehensive on-screen
...
block bootstrap default to \textbf{200 replicates} (not optimizer
iterations), and remain configurable. The optimizer's default evaluation
budget is unchanged. Two hundred replicas are a quick estimate; use more
and check stability when precise percentile endpoints matter.

All methods display a table with spectrum/function names, parameter,
original fitted value, confidence interval endpoints, optional
acceptable absolute uncertainty and assessment. Hover over the
assessment to read its explicit reasons, including the names and
correlation coefficients of strongly correlated parameters. Intervals
are not symmetric +/- errors and their midpoint is not necessarily the
original fitted value. Technical paths, seed and the baseline timestamp
remain available under \textbf{Technical details}. The confidence level
is configurable for resampling/profile; covariance shows the recorded
fit's confidence level. Changing the method recalls its most recent
recorded analysis.

Select a row and \textbf{Set acceptable uncertainty for selected
parameter\ldots{}} to enter an absolute target in that parameter's
units. Enter zero to remove it. The assessment uses the larger distance
from the original fit to either interval endpoint; it never divides by
the parameter value. Targets and reports are saved in the project.
...
\bottomrule\noalign{}
\endlastfoot
OK & Interval meets your absolute precision target and no diagnostic
issue is flagged \\
Not assessed & No precision target is set; scientific adequacy cannot be
inferred \\
Attention & Bound contact, strong correlation, insufficient precision,
failed replicates, an open profile scan, or another stated caveat \\
Critical & No usable interval, too few successful replicas,
rank-deficient covariance, or an interval spanning most of the allowed
range \\
...
wrong. The assessment tooltip explains why it is flagged, and no color
certifies the correctness of the physical model. Analyses describe their
recorded baseline; editing data or models requires refitting before
resampling. Reports remain inspectable but are marked outdated when
appropriate.
...
many replicates completed and failed. Selecting \textbf{Uncertainty
intervals, assessments and matrices} in the export dialog also writes
\texttt{uncertainty/parameter\_assessments.csv} for the recorded
analyses, including names, fitted values, interval endpoints, confidence
level, targets and reasons. Covariance/correlation matrices remain
available in the same export option.
...
Only after a crash or abnormal exit, CurveMole offers
\textbf{Recoverable sessions} at startup. Normal launches do not prompt,
even when older deferred copies exist. The same list is available from
\textbf{File \textgreater{} Recoverable sessions}. Sessions are grouped
by project name with their recovery date and copy count.
\textbf{Recover} opens the newest valid copy by default; an older
retained copy can also be selected. The recovered workspace is unsaved
and does not overwrite the original project.

\textbf{Decide later} retains the copies. \textbf{Delete recovery
copies} removes all backups for the selected session after confirmation.
Saving a project normally or explicitly choosing \textbf{Discard} when
closing or switching projects removes its recovery copies. Cancelling a
dialog or a failed save preserves them. Other projects' copies remain.
...
\textbf{Last fit: all solver/loss settings (CSV)} writes
...
\hypertarget{exporting-one-numeric-file-per-spectrum}{%
\section{14.8 Exporting one numeric file per
spectrum}\label{exporting-one-numeric-file-per-spectrum}}

Choose \textbf{File \textgreater{} Export spectra and fit
curves\ldots{}} or press \textbf{Ctrl+Shift+E} when the result should be
immediately replottable as one multi-column table per spectrum. This is
separate from the structured analysis bundle. Choose any project spectra
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curvemole} \PreprocessorTok{[{-}{-}}\SpecialStringTok{version}\PreprocessorTok{]} \PreprocessorTok{[{-}{-}}\SpecialStringTok{json}\PreprocessorTok{]} \PreprocessorTok{[{-}{-}}\SpecialStringTok{verbose}\PreprocessorTok{]}\NormalTok{ COMMAND }\PreprocessorTok{[}\SpecialStringTok{OPTIONS}\PreprocessorTok{]}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{curvemole}\NormalTok{ functions list }\AttributeTok{{-}{-}json}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curvemole}\NormalTok{ fit spectrum.csv }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}x}\NormalTok{ wavenumber }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}y}\NormalTok{ absorbance }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}function}\NormalTok{ gaussian }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}background}\NormalTok{ linear }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}output}\NormalTok{ spectrum.fitproj}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curvemole}\NormalTok{ fit{-}series scan\_01.csv scan\_02.csv scan\_03.csv }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}x}\NormalTok{ 0 }\AttributeTok{{-}{-}y}\NormalTok{ 1 }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}function}\NormalTok{ pseudo\_voigt }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}background}\NormalTok{ constant }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}mode}\NormalTok{ sequential }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}output}\NormalTok{ series.fitproj}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curvemole}\NormalTok{ inspect analysis.fitproj }\AttributeTok{{-}{-}json}
\ExtensionTok{curvemole}\NormalTok{ inspect raw\_data.dat }\AttributeTok{{-}{-}json}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curvemole}\NormalTok{ run examples/gaussian\_workflow.yml }\AttributeTok{{-}{-}json}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{curvemole}\NormalTok{ run workflow.yml }\DataTypeTok{\textbackslash{}}
  \AttributeTok{{-}{-}trust{-}plugin}\NormalTok{ org.example.lineshape}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{offset + amplitude * exp({-}x / tau)}
\end{Highlighting}
\end{Shaded}
...
normal Add dialog, Quick Add selector, and automatic peak-shape selector
without requiring a particular project to be open. Copy the folder
between computers to move a personal function library; open that folder
when CurveMole asks for the library location.
...
status badge checks updates for loaded supported plugins, lets you
select updates, and indicates when a restart is needed to activate
staged code. Checking for an update does not install it automatically.
Manual artifact download remains an alternative. Set
\texttt{CURVEMOLE\_DISABLE\_PLUGINS=1} to skip automatic loading
manually. This is failure recovery, not a sandbox for Python code.
...
After loading this plugin, choose \textbf{File \textgreater{} Exporters
\textgreater{} Laboratory CSV} (marked with the plugin symbol).
CurveMole asks for a destination and then runs the callback. The
integrated export commands stay available. The module uses NumPy,
...
folder at \texttt{custom\_plugins/your\_plugin/}, then open a pull
request to main. For private use, \textbf{Choose plugin folder\ldots{}}
installs it on your computer only; loading a plugin does not publish its
source. \textbf{Browse validated plugins} opens the downloads that have
passed the checks.

The repository's \href{../custom_plugins/README.md}{custom\_plugins
folder} contains the submission guide and a tested TSV exporter example.
Authors submit a folder with manifest, module, licence, README and
functional tests through a pull request. GitHub checks manifests, actual
registered capabilities, loading/removal and tests on Linux, Windows and
macOS. Only a successful main run produces the
\texttt{validated-community-plugins} artifact with its catalog and
tested plugin files. Download it from the Community plugins Actions
workflow, extract the chosen plugin and load it explicitly through File
\textgreater{} Plugin manager. A source folder or an open PR alone does
not indicate validation. Tests demonstrate tested behavior, not complete
safety; Python plugins still run with the user's privileges. See the
submission guide for required branch protection settings and the limits
of validation.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{https://github.com/SebRoLENS/curvemole}
\NormalTok{https://github.com/SebRoLENS/curvemole/releases}
\NormalTok{https://github.com/SebRoLENS/curvemole/issues}
\end{Highlighting}
\end{Shaded}
...
Automatic replacement is limited to a running Linux x86-64 AppImage or
Windows x86-64 standalone executable whose directory is writable. Finish
or cancel active tasks. Python/source installations, macOS packages,
unsupported architectures, and protected application directories must be
updated from the GitHub release page. On Windows, a failed staged update
records a helper log in the system temporary directory; include that log
when reporting a reproducible updater failure.
...
included in portable project copies. \textbf{Save notebook as text}
...
description directly. Clearing the description removes its icon and
notebook entry.
...
(Version 0.28.6) {[}Computer software{]}. GitHub.
...
Ctrl while dragging & Change a fixed graphical parameter without
unfixing it \\
...
\texttt{fwhm}, \texttt{eta} \\
\texttt{constant} & Constant background & \texttt{offset} \\
...
\bottomrule\noalign{}
\endlastfoot
\texttt{.fitproj} & Complete CurveMole project \\
\texttt{.fitmodel} & Reusable model and formula definitions \\
\texttt{.curvemole-function.json} & Reusable safe formula in
\texttt{my\_curvemole\_functions} \\
\texttt{.curvemole-plugin.json} & Manifest for an executable Python
plugin \\
\texttt{.yml}, \texttt{.yaml} & Reproducible workflow \\
Any text-file suffix & Imported data when the contents form a valid
numeric table \\
\texttt{.whl} & Installable Python package \\
\texttt{.AppImage} & Linux desktop package \\
\texttt{.exe} & Windows desktop executable \\
\texttt{.dmg} & macOS disk image \\
\end{longtable}

\hypertarget{appendix-d.-preview-0.28.6-limitations}{%
\chapter{Appendix D. Preview 0.28.6
limitations}\label{appendix-d.-preview-0.28.6-limitations}}
...
\item
  ...
  statistic;
\item
  Windows and macOS packages may be unsigned; Linux release provenance
  is available through GitHub artifact attestation;
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{docs/CurveMole\_User\_Manual.tex}
\NormalTok{docs/CurveMole\_User\_Manual.pdf}
\end{Highlighting}
\end{Shaded}
