...
\renewcommand{\maketitle}{%
  \begin{titlepage}
    ...
    \vspace{0.45cm}
    {\sffamily\Large\color{CurveMoleGrey}Preview 0.28.8\par}
    \vspace{0.55cm}
...
\makeatother
\subtitle{Preview 0.28.8}
\author{Sebastiano Romi}
\date{Updated for CurveMole 0.28.8}
...
CurveMole 0.28.8 Preview. The Markdown file is the authoritative source.
The LaTeX source and PDF edition are generated automatically from it and
must carry the same version number.

\begin{quote}
\textbf{Preview status.} CurveMole 0.28.8 is suitable for evaluation and
...
an independent document version. For example, a manual marked 0.28.8
describes CurveMole 0.28.8.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{chmod}\NormalTok{ +x CurveMole{-}0.28.8{-}linux{-}x86\_64.AppImage}
\ExtensionTok{./CurveMole{-}0.28.8{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{APPIMAGE\_EXTRACT\_AND\_RUN}\OperatorTok{=}\NormalTok{1 }\ExtensionTok{./CurveMole{-}0.28.8{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{shasum} \AttributeTok{{-}a}\NormalTok{ 256 CurveMole{-}0.28.8{-}macos{-}arm64.dmg}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Get}\OperatorTok{{-}}\NormalTok{FileHash }\OperatorTok{.}\NormalTok{\textbackslash{}CurveMole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{28}\OperatorTok{.}\FunctionTok{8}\OperatorTok{{-}}\NormalTok{windows}\OperatorTok{{-}}\NormalTok{x86\_64}\OperatorTok{{-}}\NormalTok{setup}\OperatorTok{.}\FunctionTok{exe} \OperatorTok{{-}}\NormalTok{Algorithm SHA256}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{python} \AttributeTok{{-}m}\NormalTok{ pip install ./curvemole{-}0.28.8{-}py3{-}none{-}any.whl}
...
\NormalTok{python }\OperatorTok{{-}}\NormalTok{m pip install }\OperatorTok{.}\NormalTok{\textbackslash{}curvemole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{28}\OperatorTok{.}\FunctionTok{8}\OperatorTok{{-}}\NormalTok{py3}\OperatorTok{{-}}\NormalTok{none}\OperatorTok{{-}}\NormalTok{any}\OperatorTok{.}\FunctionTok{whl}
...
project but is not used by the version 0.28.8 optimizer.
...
Preview 0.28.8 does not yet provide a single comprehensive on-screen
...
panel. Results are saved separately for each spectrum and method. Older
projects may need a new fit for spectra whose individual baselines were
not recorded. Profile likelihood accepts one active spectrum at a time;
coupled global fits are analyzed together when resampling is requested.
...
\bottomrule\noalign{}
\endlastfoot
OK & No diagnostic issue is flagged by these checks; this does not
establish practical adequacy \\
Attention & Bound contact, strong correlation, failed replicates, an
open profile scan, or another stated caveat \\
...
(Version 0.28.8) {[}Computer software{]}. GitHub.
...
\hypertarget{appendix-d.-preview-0.28.8-limitations}{%
\chapter{Appendix D. Preview 0.28.8
limitations}\label{appendix-d.-preview-0.28.8-limitations}}
