...
\renewcommand{\maketitle}{%
  \begin{titlepage}
    ...
    \vspace{0.45cm}
    {\sffamily\Large\color{CurveMoleGrey}Preview 0.28.12\par}
    \vspace{0.55cm}
...
\makeatother
\subtitle{Preview 0.28.12}
\author{Sebastiano Romi}
\date{Updated for CurveMole 0.28.12}
...
CurveMole 0.28.12 Preview. The Markdown file is the authoritative
source. The LaTeX source and PDF edition are generated automatically
from it and must carry the same version number.

\begin{quote}
\textbf{Preview status.} CurveMole 0.28.12 is suitable for evaluation
and controlled testing. It has not yet completed the scientific
validation planned for version 1.0.0. Inspect the residuals, parameter
...
an independent document version. For example, a manual marked 0.28.12
describes CurveMole 0.28.12.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{chmod}\NormalTok{ +x CurveMole{-}0.28.12{-}linux{-}x86\_64.AppImage}
\ExtensionTok{./CurveMole{-}0.28.12{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{APPIMAGE\_EXTRACT\_AND\_RUN=}\NormalTok{1 }\ExtensionTok{./CurveMole{-}0.28.12{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{shasum}\NormalTok{ {-}a 256 CurveMole{-}0.28.12{-}macos{-}arm64.dmg}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Get{-}FileHash .\textbackslash{}CurveMole{-}0.}\FunctionTok{28}\NormalTok{.}\FunctionTok{12}\NormalTok{{-}windows{-}x86\_64{-}setup.}\FunctionTok{exe}\NormalTok{ {-}Algorithm SHA256}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{python}\NormalTok{ {-}m pip install ./curvemole{-}0.28.12{-}py3{-}none{-}any.whl}
...
\NormalTok{python {-}m pip install .\textbackslash{}curvemole{-}0.}\FunctionTok{28}\NormalTok{.}\FunctionTok{12}\NormalTok{{-}py3{-}none{-}any.}\FunctionTok{whl}
...
project but is not used by the version 0.28.12 optimizer.
...
Preview 0.28.12 does not yet provide a single comprehensive on-screen
...
Attention\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
Bound contact, strong correlation, failed replicates, an open profile
scan, or another stated caveat\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
Critical\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
...
(Version 0.28.12) {[}Computer software{]}. GitHub.
...
\hypertarget{appendix-d.-preview-0.28.12-limitations}{%
\chapter{Appendix D. Preview 0.28.12
limitations}\label{appendix-d.-preview-0.28.12-limitations}}
