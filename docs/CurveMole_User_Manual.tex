...
\renewcommand{\maketitle}{%
  \begin{titlepage}
    ...
    \vspace{0.45cm}
    {\sffamily\Large\color{CurveMoleGrey}Preview 0.23.0\par}
    \vspace{0.55cm}
...
\makeatother
\subtitle{Preview 0.23.0}
\author{Sebastiano Romi}
\date{Updated for CurveMole 0.23.0}
...
CurveMole 0.23.0 Preview. The Markdown file is the authoritative source.
The LaTeX source and PDF edition are generated automatically from it and
must carry the same version number.

\begin{quote}
\textbf{Preview status.} CurveMole 0.23.0 is suitable for evaluation and
...
an independent document version. For example, a manual marked 0.23.0
describes CurveMole 0.23.0.
...
executable and run \\
Windows x86-64 & \texttt{CurveMole-VERSION-windows-x86\_64.exe} &
Double-click the executable \\
macOS Apple Silicon & \texttt{CurveMole-VERSION-macos-arm64.dmg} & Open
the DMG and launch the app \\
macOS Intel & \texttt{CurveMole-VERSION-macos-x86\_64.dmg} & Open the
DMG and launch the app \\
\end{longtable}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{chmod}\NormalTok{ +x CurveMole{-}0.23.0{-}linux{-}x86\_64.AppImage}
\ExtensionTok{./CurveMole{-}0.23.0{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{APPIMAGE\_EXTRACT\_AND\_RUN}\OperatorTok{=}\NormalTok{1 }\ExtensionTok{./CurveMole{-}0.23.0{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{shasum} \AttributeTok{{-}a}\NormalTok{ 256 CurveMole{-}0.23.0{-}macos{-}arm64.dmg}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Get}\OperatorTok{{-}}\NormalTok{FileHash }\OperatorTok{.}\NormalTok{\textbackslash{}CurveMole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{23}\OperatorTok{.}\FunctionTok{0}\OperatorTok{{-}}\NormalTok{windows}\OperatorTok{{-}}\NormalTok{x86\_64}\OperatorTok{.}\FunctionTok{exe} \OperatorTok{{-}}\NormalTok{Algorithm SHA256}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{python} \AttributeTok{{-}m}\NormalTok{ pip install ./curvemole{-}0.23.0{-}py3{-}none{-}any.whl}
...
\NormalTok{python }\OperatorTok{{-}}\NormalTok{m pip install }\OperatorTok{.}\NormalTok{\textbackslash{}curvemole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{23}\OperatorTok{.}\FunctionTok{0}\OperatorTok{{-}}\NormalTok{py3}\OperatorTok{{-}}\NormalTok{none}\OperatorTok{{-}}\NormalTok{any}\OperatorTok{.}\FunctionTok{whl}
...
Linux AppImage and Windows standalone installations can normally use
\textbf{Update now}. CurveMole downloads the matching release asset,
verifies the SHA-256 digest supplied by GitHub, replaces the old
executable, and removes obsolete release files. The application
directory must be writable and no fit or uncertainty task may be
running. Windows closes and restarts automatically; Linux asks the user
to close and reopen the new AppImage. Python, source, macOS, unsupported
architectures, and installations whose executable cannot be replaced
must be updated manually from the release page.
...
project but is not used by the version 0.23.0 optimizer.
...
\texttt{y} refer to the current plotted axes. \textbf{Insert column}
lists the available codes with original column names; choosing an entry
inserts its code at the cursor. \textbf{Advanced destination} chooses X,
Y or any retained column for the result.
...
\item
  choose \textbf{Y (plotted)} as the destination;
\item
...
Preview 0.23.0 does not yet provide a single comprehensive on-screen
...
Preview 0.23.0 uses 31 grid points and a one-parameter chi-square
...
session marker. Set \texttt{CURVEMOLE\_DISABLE\_PLUGINS=1} to skip
automatic loading manually. This is failure recovery, not a sandbox for
Python code.
...
project notes and a searchable description browser. Use \textbf{Add
description} from the context menu of a series, individual spectrum, or
added fit function to create or edit its description. Function
descriptions are specific to the spectrum containing that function,
including when sequential propagation preserves a component ID.
...
startup and hourly version check requests only the public latest-release
metadata from GitHub. Opening documentation or release notes,
...
(Version 0.23.0) {[}Computer software{]}. GitHub.
...
\hypertarget{appendix-d.-preview-0.23.0-limitations}{%
\chapter{Appendix D. Preview 0.23.0
limitations}\label{appendix-d.-preview-0.23.0-limitations}}
