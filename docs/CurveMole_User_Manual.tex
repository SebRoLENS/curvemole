...
\renewcommand{\maketitle}{%
  \begin{titlepage}
    ...
    \vspace{0.45cm}
    {\sffamily\Large\color{CurveMoleGrey}Preview 0.28.4\par}
    \vspace{0.55cm}
...
\makeatother
\subtitle{Preview 0.28.4}
\author{Sebastiano Romi}
\date{Updated for CurveMole 0.28.4}
...
CurveMole 0.28.4 Preview. The Markdown file is the authoritative source.
The LaTeX source and PDF edition are generated automatically from it and
must carry the same version number.

\begin{quote}
\textbf{Preview status.} CurveMole 0.28.4 is suitable for evaluation and
...
an independent document version. For example, a manual marked 0.28.4
describes CurveMole 0.28.4.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{chmod}\NormalTok{ +x CurveMole{-}0.28.4{-}linux{-}x86\_64.AppImage}
\ExtensionTok{./CurveMole{-}0.28.4{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{APPIMAGE\_EXTRACT\_AND\_RUN}\OperatorTok{=}\NormalTok{1 }\ExtensionTok{./CurveMole{-}0.28.4{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{shasum} \AttributeTok{{-}a}\NormalTok{ 256 CurveMole{-}0.28.4{-}macos{-}arm64.dmg}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Get}\OperatorTok{{-}}\NormalTok{FileHash }\OperatorTok{.}\NormalTok{\textbackslash{}CurveMole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{28}\OperatorTok{.}\FunctionTok{4}\OperatorTok{{-}}\NormalTok{windows}\OperatorTok{{-}}\NormalTok{x86\_64}\OperatorTok{{-}}\NormalTok{setup}\OperatorTok{.}\FunctionTok{exe} \OperatorTok{{-}}\NormalTok{Algorithm SHA256}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{python} \AttributeTok{{-}m}\NormalTok{ pip install ./curvemole{-}0.28.4{-}py3{-}none{-}any.whl}
...
\NormalTok{python }\OperatorTok{{-}}\NormalTok{m pip install }\OperatorTok{.}\NormalTok{\textbackslash{}curvemole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{28}\OperatorTok{.}\FunctionTok{4}\OperatorTok{{-}}\NormalTok{py3}\OperatorTok{{-}}\NormalTok{none}\OperatorTok{{-}}\NormalTok{any}\OperatorTok{.}\FunctionTok{whl}
...
panel. Tabs can still be moved or floated. CurveMole remembers window
geometry, dock arrangement, and theme. Use \textbf{View \textgreater{}
Reset layout} to restore the standard tabbed layout.
...
project but is not used by the version 0.28.4 optimizer.
...
Preview 0.28.4 does not yet provide a single comprehensive on-screen
...
acceptable absolute uncertainty, assessment and an explicit reason.
...
wrong. The table explains why it is flagged, and no color certifies the
correctness of the physical model. Analyses describe their recorded
...
(Version 0.28.4) {[}Computer software{]}. GitHub.
...
\hypertarget{appendix-d.-preview-0.28.4-limitations}{%
\chapter{Appendix D. Preview 0.28.4
limitations}\label{appendix-d.-preview-0.28.4-limitations}}
