...
\renewcommand{\maketitle}{%
  \begin{titlepage}
    ...
    \vspace{0.45cm}
    {\sffamily\Large\color{CurveMoleGrey}Preview 0.22.0\par}
    \vspace{0.55cm}
...
\makeatother
\subtitle{Preview 0.22.0}
\author{Sebastiano Romi}
\date{Updated for CurveMole 0.22.0}
...
CurveMole 0.22.0 Preview. The Markdown file is the authoritative source.
The LaTeX source and PDF edition are generated automatically from it and
must carry the same version number.

\begin{quote}
\textbf{Preview status.} CurveMole 0.22.0 is suitable for evaluation and
...
an independent document version. For example, a manual marked 0.22.0
describes CurveMole 0.22.0.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{chmod}\NormalTok{ +x CurveMole{-}0.22.0{-}linux{-}x86\_64.AppImage}
\ExtensionTok{./CurveMole{-}0.22.0{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{APPIMAGE\_EXTRACT\_AND\_RUN}\OperatorTok{=}\NormalTok{1 }\ExtensionTok{./CurveMole{-}0.22.0{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{shasum} \AttributeTok{{-}a}\NormalTok{ 256 CurveMole{-}0.22.0{-}macos{-}arm64.dmg}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Get}\OperatorTok{{-}}\NormalTok{FileHash }\OperatorTok{.}\NormalTok{\textbackslash{}CurveMole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{22}\OperatorTok{.}\FunctionTok{0}\OperatorTok{{-}}\NormalTok{windows}\OperatorTok{{-}}\NormalTok{x86\_64}\OperatorTok{.}\FunctionTok{exe} \OperatorTok{{-}}\NormalTok{Algorithm SHA256}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{python} \AttributeTok{{-}m}\NormalTok{ pip install ./curvemole{-}0.22.0{-}py3{-}none{-}any.whl}
...
\NormalTok{python }\OperatorTok{{-}}\NormalTok{m pip install }\OperatorTok{.}\NormalTok{\textbackslash{}curvemole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{22}\OperatorTok{.}\FunctionTok{0}\OperatorTok{{-}}\NormalTok{py3}\OperatorTok{{-}}\NormalTok{none}\OperatorTok{{-}}\NormalTok{any}\OperatorTok{.}\FunctionTok{whl}
...
project but is not used by the version 0.22.0 optimizer.
...
Preview 0.22.0 does not yet provide a single comprehensive on-screen
...
Preview 0.22.0 uses 31 grid points and a one-parameter chi-square
...
(Version 0.22.0) {[}Computer software{]}. GitHub.
...
\hypertarget{appendix-d.-preview-0.22.0-limitations}{%
\chapter{Appendix D. Preview 0.22.0
limitations}\label{appendix-d.-preview-0.22.0-limitations}}
