...
\renewcommand{\maketitle}{%
  \begin{titlepage}
    ...
    \vspace{0.45cm}
    {\sffamily\Large\color{CurveMoleGrey}Preview 0.2.1\par}
    \vspace{0.55cm}
...
\makeatother
\subtitle{Preview 0.2.1}
\author{Sebastiano Romi}
\date{Updated for CurveMole 0.2.1}
...
CurveMole 0.2.1 Preview. The Markdown file is the authoritative source.
The LaTeX source and PDF edition are generated automatically from it and
must carry the same version number.

\begin{quote}
\textbf{Preview status.} CurveMole 0.2.1 is suitable for evaluation and
...
an independent document version. For example, a manual marked 0.2.1
describes CurveMole 0.2.1.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{chmod}\NormalTok{ +x CurveMole{-}0.2.1{-}linux{-}x86\_64.AppImage}
\ExtensionTok{./CurveMole{-}0.2.1{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{APPIMAGE\_EXTRACT\_AND\_RUN}\OperatorTok{=}\NormalTok{1 }\ExtensionTok{./CurveMole{-}0.2.1{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{shasum} \AttributeTok{{-}a}\NormalTok{ 256 CurveMole{-}0.2.1{-}macos{-}arm64.dmg}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Get}\OperatorTok{{-}}\NormalTok{FileHash }\OperatorTok{.}\NormalTok{\textbackslash{}CurveMole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{2}\OperatorTok{.}\FunctionTok{1}\OperatorTok{{-}}\NormalTok{windows}\OperatorTok{{-}}\NormalTok{x86\_64}\OperatorTok{.}\FunctionTok{exe} \OperatorTok{{-}}\NormalTok{Algorithm SHA256}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{python} \AttributeTok{{-}m}\NormalTok{ pip install ./curvemole{-}0.2.1{-}py3{-}none{-}any.whl}
...
\NormalTok{python }\OperatorTok{{-}}\NormalTok{m pip install }\OperatorTok{.}\NormalTok{\textbackslash{}curvemole}\OperatorTok{{-}}\NormalTok{0}\OperatorTok{.}\FunctionTok{2}\OperatorTok{.}\FunctionTok{1}\OperatorTok{{-}}\NormalTok{py3}\OperatorTok{{-}}\NormalTok{none}\OperatorTok{{-}}\NormalTok{any}\OperatorTok{.}\FunctionTok{whl}
...
After a successful fit, the model line and residual panel update.
Parameter standard errors appear in the \texttt{±1\ sigma} column when
covariance is available. Open \textbf{View \textgreater{} Diagnostics}
to inspect residual warnings.
...
a new empty directory for the first export. The bundle contains
human-readable CSV tables, fit parameters, plots, HTML and PDF reports,
machine-readable JSON, a reusable model, and a portable project copy.
...
column becomes a separate curve sharing the selected x column. The
Python API additionally supports explicit x-y column pairs.
...
project but is not used by the version 0.2.1 optimizer.
...
Choose how many suggestions to add. Version 0.2.1 creates Gaussian
...
the active curve. Options include:
...
Internal curve and component identifiers are preserved in
\texttt{.fitproj}, YAML-derived objects, Python objects, and
machine-readable exports. In Preview 0.2.1, the GUI does not yet provide
a dedicated path picker. Complex cross-curve links are therefore most
reliably prepared through the Python API or a reproducible workflow and
then inspected in the GUI.
...
\tightlist
\item
  confirm the active and selected curves;
\item
...
Preview 0.2.1 does not yet provide a single comprehensive on-screen
...
Preview 0.2.1 uses 31 grid points and a one-parameter chi-square
...
its recovery files. Preview 0.2.1 does not yet show an automatic
...
\textbf{Ctrl+E}. For a first export, select an empty directory. Options
are:

\begin{itemize}
\tightlist
\item
  \textbf{Create versioned export:} create a timestamped subdirectory;
\item
  \textbf{Update existing CurveMole-owned files after confirmation:}
  update only files listed in the existing export manifest;
\item
  \textbf{Include full uncertainty samples:} preserve raw samples in the
  project copy at the cost of file size.
\end{itemize}

\hypertarget{safe-overwrite-policy}{%
\section{14.2 Safe overwrite policy}\label{safe-overwrite-policy}}

The hidden \texttt{.curvemole-export.json} manifest identifies files
owned by a prior CurveMole export. CurveMole refuses to overwrite
matching files if:
...
\hypertarget{bundle-layout}{%
\section{14.3 Bundle layout}\label{bundle-layout}}

The exact set depends on available fit and covariance results. A typical
bundle is:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{README.txt}
\NormalTok{parameters.csv}
\NormalTok{CURVE\_wide.csv}
\NormalTok{main\_plot.png}
\NormalTok{summary\_report.html}
\NormalTok{PROJECT.fitmodel}
\NormalTok{PROJECT.fitproj}
\NormalTok{data/}
\NormalTok{  CURVE\_wide.csv}
\NormalTok{python/}
\NormalTok{  data\_tidy.csv}
\NormalTok{  results.json}
\NormalTok{figures/}
\NormalTok{  main\_plot.svg}
\NormalTok{report/}
\NormalTok{  full\_reproducibility.html}
\NormalTok{  summary.pdf}
\NormalTok{uncertainty/}
\NormalTok{  covariance.csv}
\NormalTok{  correlation.csv}
\NormalTok{diagnostics/}
\NormalTok{  CURVE\_ID\_autocorrelation.csv}
\NormalTok{.curvemole{-}export.json}
\end{Highlighting}
\end{Shaded}

\hypertarget{wide-data-tables}{%
\section{14.4 Wide data tables}\label{wide-data-tables}}

Each wide CSV is intended for direct inspection and tools such as Origin
...
\section{14.5 Tidy data and JSON}\label{tidy-data-and-json}}
...
\hypertarget{parameter-table-1}{%
\section{14.6 Parameter table}\label{parameter-table-1}}

\texttt{parameters.csv} contains component and parameter identifiers,
values, standard errors, confidence limits, bounds, state, link, unit,
human-readable value with error, and derived area and FWHM where
defined.
...
(Version 0.2.1) {[}Computer software{]}. Zenodo.
https://doi.org/10.5281/zenodo.22114004
\end{quote}

The repository contains \texttt{CITATION.cff}, and GitHub provides
\textbf{Cite this repository}. Future releases receive their own version
DOI while remaining associated with the same software concept.
...
\hypertarget{appendix-d.-preview-0.2.1-limitations}{%
\chapter{Appendix D. Preview 0.2.1
limitations}\label{appendix-d.-preview-0.2.1-limitations}}
