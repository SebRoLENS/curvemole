...
\renewcommand{\maketitle}{%
  \begin{titlepage}
    ...
    \vspace{0.45cm}
    {\sffamily\Large\color{CurveMoleGrey}Preview 0.8.2\par}
    \vspace{0.55cm}
...
\makeatother
\subtitle{Preview 0.8.2}
\author{Sebastiano Romi}
\date{Updated for CurveMole 0.8.2}
...
CurveMole 0.8.2 Preview. The Markdown file is the authoritative source.
The LaTeX source and PDF edition are generated automatically from it and
must carry the same version number.

\begin{quote}
\textbf{Preview status.} CurveMole 0.8.2 is suitable for evaluation and
...
an independent document version. For example, a manual marked 0.8.2
describes CurveMole 0.8.2.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{chmod}\NormalTok{ +x CurveMole{-}0.8.2{-}linux{-}x86\_64.AppImage}
\ExtensionTok{./CurveMole{-}0.8.2{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{APPIMAGE\_EXTRACT\_AND\_RUN=}\NormalTok{1 }\ExtensionTok{./CurveMole{-}0.8.2{-}linux{-}x86\_64.AppImage}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{shasum}\NormalTok{ {-}a 256 CurveMole{-}0.8.2{-}macos{-}arm64.dmg}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Get{-}FileHash .\textbackslash{}CurveMole{-}0.}\FunctionTok{8}\NormalTok{.}\FunctionTok{2}\NormalTok{{-}windows{-}x86\_64.}\FunctionTok{exe}\NormalTok{ {-}Algorithm SHA256}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{python}\NormalTok{ {-}m pip install ./curvemole{-}0.8.2{-}py3{-}none{-}any.whl}
...
\NormalTok{python {-}m pip install .\textbackslash{}curvemole{-}0.}\FunctionTok{8}\NormalTok{.}\FunctionTok{2}\NormalTok{{-}py3{-}none{-}any.}\FunctionTok{whl}
...
project but is not used by the version 0.8.2 optimizer.
...
Choose how many suggestions to add. Version 0.8.2 creates Gaussian
...
Preview 0.8.2 does not yet provide a single comprehensive on-screen
...
Preview 0.8.2 uses 31 grid points and a one-parameter chi-square
...
its recovery files. Preview 0.8.2 does not yet show an automatic
...
(Version 0.8.2) {[}Computer software{]}. GitHub.
...
\hypertarget{appendix-d.-preview-0.8.2-limitations}{%
\chapter{Appendix D. Preview 0.8.2
limitations}\label{appendix-d.-preview-0.8.2-limitations}}
